...
<!-- Авторизация через VK ID 
    ...
    (получать учтные данные через backend)
    поэтому использую вариант авторизации с VK ID low-code (код на js который предоставляет vk).
    Если получить реальный email, то по нему можно проводить и регистрацию и авторизацию (или только авторизацию, 
    а регистрацию по форме, чтобы были полные данные), но у меня не подтверждается профиль, для тестирования сделаю так:
    ...
    При регистрации или авторизации через VK ID делаем проверку по email и если есть совпадение, то делаем update учетной записи с добавлением VK ID и не регаем нового пользователя
    При регистрации через форму проверяем не только по логину, а еще по email, возможно пользователь уже регистрировался по VK ID 
-->
        ...
        if ('VKIDSDK' in window) {
            ...
            function vkidOnSuccess(data) {
                ...
                VKID.Auth.userInfo(data.access_token).then((result) => { // получаем данные пользователя по access_token
                    ...
                    // не разобрался как передать данные и вызвать метод контроллера,
                    // поэтому реализовал извращенный вариат - генерируем форму передаем данные и csrf токен
                    const form = document.createElement('form');
                    form.setAttribute("method", "post");
                    form.setAttribute("action", "/registration/signupvk");
                    ...
                    // приведенные ниже способы данные передают, но я не понимаю как мне перейти в (запустить) контроллер и метод
